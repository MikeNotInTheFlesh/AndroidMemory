...
void setup() {
  ...
  myFont = createFont("impact.ttf", 64, true);
  textFont(myFont);
  ...
  descriptor = context.getAssets().openFd("correctPlay.mp3");
  descriptor2 = context.getAssets().openFd("levelComplete.mp3");
 
  correctPlay = soundPool.load(descriptor, 1);
  levelComplete = soundPool.load(descriptor2, 1);
...
void checkGuess(int id){
  if (firstGuessId == -1){
    ...
    soundPool.play(correctPlay, 0.9f, 0.9f, 2, 0, 1.0);
...
void correctGuess() {
  if (cards.size() == 2){
    winFlag = true;
    soundPool.play(levelComplete, 0.9f, 0.9f, 2, 0, 1.0);
    return;
  }
  for (int i = cards.size() - 1; i >= 0; i--) {
...
void winner(){
  ...
  //text("Time: "+ str(timer / 30), width / 2, height / 2);
